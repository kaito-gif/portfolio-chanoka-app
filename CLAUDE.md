...
### 最優先: カート描画固着の不具合を解決する

上記「未解決の不具合」を参照。原因(Cart Transform の expand)は特定済みだが対策は未定。
`docs/requirements.md` の「原因の切り分け」に挙げた2択(Shopifyへ報告して待つ／expandを
やめて料金を独立行にする)のどちらに進むか、着手前にユーザーと相談すること。
**構成要素#1は「保存はできるが表示が古いままになりうる」状態で止まっている**ため、
これを解決しないままデモ資材(見せ方)の作成に進まない。
