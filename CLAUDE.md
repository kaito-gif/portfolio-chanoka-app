...
**未解決の不具合が1件ある**: **Cart Transform の expand で展開された行は、
カートページのプロパティ表示が更新されない**(2026-08-14 に原因を切り分け済み。
展開の有無だけを切り替えて再現。未公開テーマ・app dev・本アプリのJSはいずれも無関係)。
金額は常に正しいが、**「表示だけの問題」ではない**。古いHTMLで描かれるとブロックの
`data-line-key` も古くなり、保存が不発になる(この二次被害は行の特定を
key→index＋variant の2段構えにして塞いだ)。
アプリ側からの回避策は見つかっていない。**公開前に必ず決着させること。**
