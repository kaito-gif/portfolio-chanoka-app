...
- GitHub: `kaito-gif/portfolio-chanoka-app`(現在private。公開前リーク検査は完了済み、
  public化はユーザー判断待ち)。`.github/workflows/test.yml` でfixtureテストをCI実行する
...
**フェーズ1・フェーズ2とも完了した。** 構成要素#1〜#11すべて実装・実機検証済み
（購入者のカート熨斗指定→料金加算→包装料無料化→注文確定→受注画面での確認・訂正→
購入者の注文ステータスページでの確認、まで一通り成立）。デモ資材(`docs/demo/`)・
...
GitHubリポジトリ `kaito-gif/portfolio-chanoka-app` を作成済み（現在private）。
公開前のリーク検査(ng-words.txt)は完了しクリーンだった。**public化はユーザー判断待ち**
（`gh repo edit --visibility public`でいつでも切り替え可能）。

### 未解決のまま残っている既知の問題(次に触るときに再現を試みること)

- **名入れの値が1回だけ注文確定後に空文字になった事象**(2026-08-15)。日本語IME変換確定前に
  保存ボタンが押された可能性を疑っているが再現条件は未特定。詳細は `docs/spec.md`
  の「既知の未検証事項」
- **`reconcileFeeLines`が数量変更に追随しないケースを1回観測**(2026-08-16、デモ撮影中)。
  Dawn標準の`+`ボタンでカート数量を増やした直後、のし代・包装料の行数量が親行に追随せず
  `1`のまま残った。チェックアウト到達時点では正しい数量に補正されていたため実害は
  未確認。再現条件不明。詳細は `docs/context.md` の該当ログ
...
2026-08-16時点で残っているのは以下のみ:

1. リポジトリのpublic化(ユーザー判断待ち)
2. 上記「未解決のまま残っている既知の問題」の2件の再現・解消(優先度低)
3. フェーズ3(配送希望日時・メッセージカード文面等)は要件定義時点で「当面やらない」と
