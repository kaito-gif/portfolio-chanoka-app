...
4. **リスク1**(次はここ): Cart Transform の expand operation を実装し、Dawn 標準のカート表示と
   競合しないか確認する。ここが崩れたら「確定事項」の料金設計から見直しになる。
   **最初に「販売チャネル未公開の variant を expand のコンポーネントにできるか」を確かめる**
   (駄目ならリスク2で作った商品の非公開方針から見直し)
5. 以降、リスク3・4・7・8・9・10 を順不同で潰す
