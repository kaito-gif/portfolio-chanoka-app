...
最終更新: 2026-08-16（公開前のリーク検査を実施しクリア。設計書三層をFR-16/FR-18の実装完了へ追随させた。残るはリポジトリのpublic化の判断のみ）
...
| 11 | デモ資材の作成 | ✅ 実装済み（2026-08-16）。`docs/demo/`にGIF3件・スクリーンショット2件。README.mdから参照 |
...
2. Customer Account拡張のdev previewは「その顧客アカウントが実際に注文を持っている」
   ことが前提(`no_order=true`で弾かれる)。ユーザーの実アカウントでサインインし、
   テスト注文(#1008、銀行振込・実口座への振込不要なデモ決済)を
   実際に作成して検証した
...
## 構成要素#11の実装結果（デモ資材の作成、2026-08-16）

`docs/demo/`にGIF3件・スクリーンショット2件を作成し、`README.md`から参照する形にした。

| ファイル | 内容 |
|---|---|
| `01-cart-noshi-fee.gif` | カートで熨斗を指定→のし代・包装料が独立行として加算される |
| `02-wrap-fee-free.gif` | 数量を増やして閾値到達→チェックアウトで包装料が無料になる |
| `03-admin-noshi-card.png` | 受注画面の熨斗情報カード・印字用データ（訂正済みバッジ付き） |
| `04-admin-correction.gif` | 受注後の訂正操作（表書きを書き換え→印字用データに即反映） |
| `05-customer-order-status.png` | 購入者側の注文ステータスページの熨斗情報カード（構成要素#7） |

**撮影中に気づいた点(未解決・実害は限定的)**: カート数量をDawn標準の`+`ボタンで
増やした直後、のし代・包装料の行数量が親行の数量に追随せず`1`のまま残るケースを
1回観測した（本来`reconcileFeeLines`が追随させる設計。詳細設計5章参照）。
チェックアウトへ進んだ時点では正しい数量に補正されていたため実害は確認していないが、
再現条件は特定できていない。次回このあたりを触るときに再現を試みること。

**GIF撮影時の制約**: カート見積もり合計（`/cart`ページ）はディスカウントを含まない
（「ディスカウントと配送料はチェックアウト時に計算されます」とUI上に明記されている）ため、
「包装料が無料になる瞬間」はチェックアウトページで撮影する必要があった。

## `README.md`の書き換え結果（2026-08-16）

Shopifyテンプレートの雛形（App Home・Preact/Vite・GraphQL Codegenの説明など、現在の
実装と無関係な内容）から、実態に即した内容へ全面的に書き換えた。デモ資材への導線、
拡張機能構成の一覧、`docs/`各文書への導線を追加した。

## 公開前のリーク検査の結果（2026-08-16）

`~/.claude/portfolio/ng-words.txt`（コメント・空行除去後122語）でリポジトリ全体・
コミットメッセージ全履歴・ファイル名一覧を検査した。ヒットは1件のみで、
`docs/design/01-requirements.md`内の「**クラシック**顧客アカウント」という語句の中の
「ラシック」がNGワード「ラシック」と部分一致した誤検出（実在の氏名・屋号等とは無関係）。
実在情報のリークはなし。
...
- **名入れが消える事象(2026-08-15)への対応**: `compositionstart`/`compositionend`で
  名入れ入力欄のIME変換中フラグを持ち、変換中に保存ボタンがクリックされた場合は
  `compositionend`まで`save()`を遅延させるガードを追加した(`noshi-cart.js`)。
  実機で「変換中にクリック→`compositionend`まで`/cart/change.js`が発火しないこと・
  確定後に正しい値で保存されること」を確認済み。ただし元の事象自体の再現条件は
  依然として未特定のため、これが根本原因だったと断定はできない(再発時は経過観察)
- **`reconcileFeeLines`が数量変更に追随しないケース(2026-08-16)への対応**: 原因は
  `passiveReconcile`の`reconciling`フラグが「実行中に来た呼び出しを捨てる」実装で、
  ページ読み込み直後の初回reconcileがネットワーク往復中に数量変更が来ると
  その分が握りつぶされていたこと。実行中の呼び出しを1回分キューして完了後に
  再実行するよう修正した。実機でDawnの`+`ボタンを連打し、fee行の数量が
  親行に追随することを確認済み
