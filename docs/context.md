...
2026-08-13: リスク9は調査のみで中断。税務の解釈（茶葉8% / のし・包装料10%）は
国税庁の資料と整合することを確認したが、Shopify 側で (a) バンドルへの税の上書きが
無視される可能性、(b) 開発ストアが税抜表示・請求先US のままで日本の消費税を扱う
状態にない、の2点が残っている。(b) は第1弾のデモストアの価格表示に影響するため、
判断を仰いでいる。
