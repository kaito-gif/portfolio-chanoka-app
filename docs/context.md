...
2026-08-15: 構成要素#9（受注後の熨斗の訂正・FR-18）を実装し、フェーズ2の残りは
構成要素#7・#10・#11の3項目のみになった。スコープを`read_orders`から`write_orders`へ
差し替え、Order metafield定義`[order.metafields.app.noshi_correction]`（type: json）を
`shopify.app.toml`に追加した。`shopify app dev`起動でこの定義が自動デプロイされることを
GraphiQL経由の`metafieldsSet`/読み出しクエリで確認した。**当初懸念していた「保護対象顧客
データの壁」（構成要素#5で判明した`read_orders`単体では突破できない制約）は、
`write_orders`への差し替え時には再発しなかった**（インストール済みアプリのProtected
Customer Data承認は個別スコープ単位ではなく一度承認すれば足りるとみられる）。

実機で注文2件（#1005・#1006）に対し、Admin画面から実際に「訂正する」ボタン→
テキストフィールド編集→保存の一連の操作を行い、以下を確認した:
- 既存の訂正値がある行を再編集する際、フォームに正しい初期値が入ること
- 名入れを空文字に訂正した場合も正しく保存・表示されること（表示は非表示、
  フォームには空文字として保持）
- 訂正がまだ無い注文での初回訂正（Order metafieldの初期値`{}`へのマージ）が
  正しく動くこと
- 2つの注文の訂正値が互いに独立していること
- キャンセルボタンではmetafieldへの書き込みが発生しないこと
- 保存後、印字用データ（FR-17）にも訂正後の値が反映されること

副産物として、訂正UI追加でブロックの表示行数が増え、Admin側の「警告：このブロックは
高すぎます」という制約に触れることが分かった。この警告時、Adminはブロック内容を
...
2026-08-16: 構成要素#11（デモ資材の作成）とREADME.mdの書き換えを実施し、**フェーズ2が
完了した**。残るのは公開前のリーク検査(ng-words.txt)のみ。

`docs/demo/`にGIF3件（カートでの熨斗指定・料金加算、包装料無料化、受注後の訂正操作）と
スクリーンショット2件（受注画面の熨斗カード・印字用データ、購入者側の注文ステータス
ページ）を作成した。撮影は`chanoka-demo`の実機で行った（`shopify app dev`起動、
実際にテスト注文を作成）。

撮影中に2つの気づきがあった:
- カート見積もり合計（`/cart`ページ）はディスカウントを含まないため、「包装料無料化の
  瞬間」はチェックアウトページで撮影する必要があった
- Dawn標準の`+`ボタンでカート数量を増やした直後、のし代・包装料の行数量が親行に
  追随せず`1`のまま残るケースを1回観測した（未解決・再現条件未特定。チェックアウト
  到達時点では正しい数量に補正されていたため実害は未確認）。次回このあたりを
  触るときに再現を試みること

`README.md`はShopifyテンプレートの雛形（App Home・GraphQL Codegenの説明など、現在の
実装と無関係な内容）のままだったため、実態に即した内容へ全面的に書き換えた。

2026-08-16: 未解決だった既知の問題2件に対応した。

`reconcileFeeLines`が数量変更に追随しないケースの原因を特定した。`passiveReconcile`の
`reconciling`フラグは「実行中なら何もしない」だけで、実行中に来た呼び出しを再実行せず
そのまま捨てていた。ページ読み込み直後の初回reconcileがネットワーク往復中に
Dawnの`+`ボタンでMutationObserverが発火すると、その数量変更分の再整合が丸ごと
握りつぶされる。実行中の呼び出しは「完了後にもう一度だけ最新状態で整合し直す」よう
1回分キューするよう修正した。`chanoka-demo`実機で`+`ボタンを連打して検証し、
fee行(のし代・包装料)の数量が親行に追随することを確認した。

名入れが注文確定後に空文字になった事象については、疑っていた「IME変換確定前に
保存ボタンが押される」経路に対してガードを追加した。`compositionstart`/`compositionend`で
名入れ入力欄の変換中状態を追跡し、変換中に保存ボタンがクリックされたら
`compositionend`まで`save()`を遅延させる。実機でJSからcompositionイベントを
synthetic発火させて検証し、変換中は`/cart/change.js`が発火しないこと・
...
2026-08-17: 第三者レビュー（portfolio-mentor経由）を受け、READMEに GitHub Actions の
CIステータスバッジ（`test.yml`）を追加。あわせてNotion公開ページの「検証の進め方」に
実行ログへのリンクを追加した。本アプリの成果はAdmin/チェックアウト/注文ステータス
ページ内でしか見えず、画面録画・スクショだけでは動作の裏取りがしにくいという指摘への対応。
