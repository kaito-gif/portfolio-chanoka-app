...
| `data-line-index` | 描画時の並び順（0 始まり） | 行特定の第 2 候補 |
...
含まれるアプリ ID を、**リポジトリ内でこのファイルだけが知っている**状態にするため。
アプリを作り直した際の修正箇所を 1 箇所に閉じ込める。
...
existing = existingFeeLines(cart)      # _noshi_parent_key を持つ料金行
...
if toRemove または toUpdate:
    POST /cart/update.js { updates: { 行キー: 数量, … } }    # 1 リクエストにまとめる

if toAdd:
    POST /cart/add.js { items: [ { id, quantity, properties: { _noshi_parent_key } }, … ] }
...
第 2 候補が成立する前提は、本ブロックがカートの商品行を**そのままの順序で描画している**こと。
...
（テーマは**読むだけで改変しない**）。カートの並び順と描画順が対応することが前提。
