...
本章は**本設計書が定める仕様と、現在の実装が食い違っている箇所**を記録する。
いずれも設計レビュー（2026-08-14）で検出したもので、修正は未着手。
修正時は `assets/noshi-cart.js` の変更を伴うため、実機での通し確認が必要になる。

### 7.1 料金行の追加・更新の失敗を検知していない
...
| あるべき仕様 | 3.4.4 の ※2 |
| 対処 | レスポンスを検証し、失敗時は例外にする。能動整合では再読み込みせずエラー表示に倒す |

### 7.2 不正に追加された料金行が除去されない
